...
arpis_socket::arpis_socket(int domain, int type, int protocol) {
    if (this->socket_ = socket(domain, type, protocol)) {
        std::cout << "error creating socket" << std::endl;
    }
}

int arpis_socket::get_socket() {
    return this->socket_;
}

void arpis_socket::register_sockaddr(sockaddr_in * sockaddr) {
    memset(sockaddr, '\0', sizeof(*sockaddr));
    this->sockaddr_.push_back(sockaddr);
}

sockaddr_in * arpis_socket::get_sockaddr(int id) {
    return this->sockaddr_.at(id);
}

int arpis_socket::sock_bind(int sockid) {
